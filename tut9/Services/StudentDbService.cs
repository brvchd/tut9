...
namespace tut9.Services
{
    public class StudentDbService : IStudentDbService
    {
        ...
        public PromoteStudentsResponse PromoteStudents(PromoteStudentsRequest request)
        {
            ...
            var enrollment = _context.Enrollment.Where(en => en.Semester == request.Semester && en.IdStudy == idStudy).FirstOrDefault();
            if (enrollment == null)
            {
                throw new NoStudentsToPromote();
            }

            var enrollments = _context.Enrollment.Where(enr => enr.Semester == request.Semester).ToList();
